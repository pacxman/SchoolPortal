...
const createOnField = ({ castTypes, limits }, entries) => (name, rawValue, info) => {
    ...
    if (valueTruncated) {
        return void entries.emit("error", (0, requestEntityTooLarge_1.default)(`Field size limit exceeded: Available up to ${limits.fieldSize} bytes per field.`));
    }
    entries.enqueue();
    try {
        const path = (0, getFieldPath_1.default)(name);
        const value = castTypes ? (0, castType_1.default)(String(rawValue)) : rawValue;
        entries.pull([
            path,
